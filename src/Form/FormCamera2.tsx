...
const FormCamera2 = ({ navigation, route }: Props) => {
  ...
  const takePicture = async () => {
    if (hasPermission) {
      ...
      if (!result.canceled) {
        ...
        await saveImagesToForm('formulario7Images', newPhotos);
      }
    } else {
  ...
  const deletePhoto = async (index) => {
    ...
    await saveImagesToForm('formulario7Images', updatedPhotos);
  };

  const retakePhoto = () => {
    Alert.alert('Atenção', 'Deseja realmente descartar a foto?', [
  ...
  const handleNext = () => {
    navigation.navigate('Fotos dos vidros', { photoCountRef: photoCountRef.current });
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Fotos da Placa</Text>
